  ...
  async function createCourse() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/instructor/courses?courseId=${c.id}`);
      } else {
                    ...
                    {visible.map(c => (
                      <li key={c.id} className="p-4 bg-white dark:bg-gray-800 rounded shadow flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => router.push(`/instructor/courses?courseId=${c.id}`)} className="px-3 py-2 border rounded-md text-sm text-[#0A66C2]">Manage</button>
                          <button onClick={() => router.push(`/instructor/courses/edit?courseId=${c.id}`)} className="px-3 py-2 border rounded-md text-sm">Edit</button>
