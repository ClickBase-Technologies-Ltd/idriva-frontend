...
export default function FeedJob({ job }) {
  ...
  const handleSubmitApplication = async (e) => {
    ...
    try {
      await api.post(
        `${process.env.NEXT_PUBLIC_API_URL}/jobs/${job.jobId}/apply`,
        formData,
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm break-words">
      ...
      <div className="flex items-start mb-3">
        ...
        <div>
          <p className="font-semibold text-sm">{job?.company?.companyName}</p>
      ...
      <div className="mt-4 flex justify-between items-center text-xs text-gray-500 dark:text-gray-400">
        <span className="font-bold">{job?.jobLocation || "Location not specified"}</span>
        <span className="font-bold">Salary: N{job.salary ? job.salary.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") : "Not specified"}</span>
